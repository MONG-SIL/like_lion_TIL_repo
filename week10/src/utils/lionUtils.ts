...
export function picsumUrl(seed: string): string {
...
export function prepareInitialLions(initialLions: LionSeed[]): Lion[] {
  const now = Date.now();
...
export function lionToRow(lion: Lion): LionInsert {
  return {
    id: lion.id,
    is_self: lion.isSelf,
    photo_url: lion.photoUrl,
    badge: lion.badge,
    name: lion.name,
    part: lion.part,
    tagline: lion.tagline,
    organization: lion.organization,
    intro: lion.intro,
    skills: lion.skills,
    contacts: lion.contacts,
    quote: lion.quote,
...
export function getVisibleLions(lions: Lion[], { part, sortBy, searchName }: ViewOptions): Lion[] {
  let list = [...lions];
...
export function lionFromApiUser(user: RandomUser): Lion {
  ...
  const part = randomPick(VALID_PARTS);

  const skillBank: Record<LionPart, string[]> = {
...
export function lionFromFormFields(fields: LionFormFields & { skills: string }): Lion {
...
export function formFieldsFromLion(lion: Lion): LionFormFields {
...
export function validateFormFields(fields: LionFormFields): ValidationResult {
  const errors: ValidationResult['errors'] = {};
  let ok = true;

  const required: [keyof LionFormFields, string, string][] = [
  ...
  if (fields.part && !VALID_PARTS.includes(fields.part as LionPart)) {
    ok = false;
...
export function isFormComplete(fields: LionFormFields): boolean {
  return (
    Boolean(normalizeText(fields.name)) &&
    Boolean(normalizeText(fields.part)) &&
    Boolean(normalizeText(fields.skills)) &&
    Boolean(normalizeText(fields.tagline)) &&
    Boolean(normalizeText(fields.intro)) &&
    Boolean(normalizeText(fields.email)) &&
    Boolean(normalizeText(fields.phone)) &&
    Boolean(normalizeText(fields.website)) &&
    Boolean(normalizeText(fields.quote))
  );
}
